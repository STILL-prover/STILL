-- Demonstrates the proof of the axiom of choice in the system of dependent session types or just ECC.
module AxiomOfChoice.AC where
...
axiomOfChoiceProof = let
    t = _Theorem (_Init "AC") [] "axiomOfChoice" axiomOfChoice
...
axiomOfChoiceInOnlyECC = let
    t = _Theorem (_Init "ACF") [] "axiomOfChoiceFunctional" $ axiomOfChoiceFunctional (Type 1) (Type 1)
    s1 = _Apply t _FTermR
...
axiomOfChoiceTermProof = let
    t = Right $ FT._FTheorem Data.Map.empty (axiomOfChoiceTermProp (Type 1) (Type 1)) S.empty :: Either String (FT.ProofState)
